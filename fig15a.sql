select cont.name, pub2.venue, count(distinct pubPub.dblp_key) from pub_data_publication pubPub, pub_data_authoraffiliation pubAutAff, pub_data_author pubAuth, pub_data_publication_authors
...
and (pub2.venue= 'PODS' or pub2.venue = 'SIGMOD')-- Select either sigmod or papers 
...
group by cont.name, pub2.venue; -- (This will be set to specific value for each of the queries but for now is easier for us to just see all to verify original q
